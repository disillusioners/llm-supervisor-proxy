...
# Test script for OpenAI Format Internal Path WITH Tool Call Buffering
# Validates full OpenAI format support: tool calls, reasoning, DONE marker, finish_reason
# This version has TOOL_CALL_BUFFER_ENABLED (default) - tool calls are buffered and repaired
...
echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}  OpenAI Format Internal Path Tests (BUFFERED)  ${NC}"
echo -e "${BLUE}  Tool Call Buffering: ENABLED (default)         ${NC}"
...
export UPSTREAM_URL="http://localhost:4003"
...
# NOTE: TOOL_CALL_BUFFER_DISABLED is NOT set - buffering is ENABLED (default)
...
for i in 1 2 3; do
    curl -s -X DELETE "http://localhost:$PROXY_PORT/fe/api/models/mock-openai-model-buffered" 2>/dev/null || true
    sleep 0.3
done
sleep 0.5
for i in 1 2 3; do
    curl -s -X DELETE "http://localhost:$PROXY_PORT/fe/api/credentials/mock-openai-cred-buffered" 2>/dev/null || true
    sleep 0.3
done
sleep 1
...
# With buffering ENABLED, tool repair happens during buffering phase
echo -e "\n${YELLOW}[9/18] Test 6: Streaming Tool Call with Malformed JSON (Buffered Tool Repair)${NC}"
...
# Verify that tool repair was applied (buffered mode should also repair malformed JSON)
...
# Summary
echo -e "\n${BLUE}=================================================${NC}"
echo -e "${BLUE}           Test Summary (Buffered Mode)         ${NC}"
echo -e "${BLUE}=================================================${NC}"
echo -e ""
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo -e ""
    echo -e "${GREEN}OpenAI Format Support Verified (with Tool Call Buffering):${NC}"
    ...
    echo -e "  ${YELLOW}✓${NC} Buffered tool repair (malformed JSON)"
    echo -e ""
