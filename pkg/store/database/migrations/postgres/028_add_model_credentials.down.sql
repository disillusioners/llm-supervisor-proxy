...
-- pre-029-deprecation case). PG's `ALTER TABLE ... ADD COLUMN IF
-- NOT EXISTS` is supported on PG 9.6+, which the project's PG
-- target satisfies. For now, assume the down-migration runs in a
-- state where credential_id EXISTS (pre-029 typical), so we skip
-- the ADD. If the column is absent, the operator must run a manual
-- ALTER TABLE first.
...
WHERE credentials_json IS NOT NULL
  ...
  AND (credentials_json::JSONB -> 0 ->> 'credential_id') IS NOT NULL;
