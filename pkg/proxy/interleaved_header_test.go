...
func TestParseInterleavedThinkingHeaderValue(t *testing.T) {
	cases := []struct {
		...
		// accepted (per handler.go:465 precedent)
		{"true", true},
		{"1", true},
		// rejected (case-sensitive; matches X-Force-Ultimate-Model semantics)
		{"True", false},
		{"TRUE", false},
		{"yes", false},
		{"YES", false},
...
func TestParseInterleavedThinkingHeader(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			setup: func(r *http.Request) {
				r.Header.Set(InterleavedThinkingHeader, "true")
			},
			want: true,
		},
		{
			...
			setup: func(r *http.Request) {
				r.Header.Set(InterleavedThinkingHeader, "1")
			},
			want: true,
		},
		{
		...
		},
		{
			name: "yes rejected",
			setup: func(r *http.Request) {
				r.Header.Set(InterleavedThinkingHeader, "yes")
			},
			want: false,
		},
		{
			...
			setup: func(r *http.Request) {
				r.Header.Set(InterleavedThinkingHeader, "")
			},
			want: false,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "nil request" {
				if got := parseInterleavedThinkingHeader(nil); got != tc.want {
					t.Errorf("nil req: got %v, want %v", got, tc.want)
				}
				return
			}
			tc.setup(r)
