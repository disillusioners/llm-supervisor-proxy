...
import type { ModelUsageDataRow } from '../../types';
...
export function ModelUsageChart({ data, view, loading, metricType = 'tokens' }: ModelUsageChartProps) {
  ...
  useEffect(() => {
    ...
    // Create datasets: one per model + one total line
    ...
    // Add a line for each unique model
    const getMetricValue = (row: ModelUsageDataRow | undefined) => {
      if (!row) return 0;
      return metricType === 'tokens' ? row.total_tokens : row.request_count;
    };

    modelIds.forEach((modelId, index) => {
      const modelData = data.find(d => d.model_id === modelId);
      const color = COLORS[index % COLORS.length];
      datasets.push({
        label: modelData?.model_name || modelId,
        data: buckets.map(bucket => {
          const row = data.find(d => d.hour_bucket === bucket && d.model_id === modelId);
    ...
    // Add Total line (sum of all models per bucket)
    datasets.push({
      label: 'Total',
      data: buckets.map(bucket => {
        return data
