...
| DB Cache Layer MVP Gate (@ 42d98f3→a753698, re-gate @ a09877d) | 2026-08-29 | ✅ **READY-TO-MERGE** (re-gate) | Initial gate (14 workers): full `-race` sweep 12/12 slices green; E2E real-SQLite-corruption outage pack FAIL ×3 → F1 (HIGH, SQLite shapes not infra-classified → 401s >60s into outage) + F2/F3 (credential write-through ≤60s) + F4-F7 documented; flake stabilized c9559cf; S3 anthropic bug fixed on branch; ensure.md Critical 4/4. **Re-gate @ a09877d (7 workers)**: pack A **PASS exit 0** (28/28 through outage, stale-tier 200s t70/t90, zero misroutes, recovery 61s), pack B **PASS** (attach first-try, rotation iter-1, zero stale-key), diff review 0 blockers, full `-race` sweep green again (5 slices, modelscache `-count=10` 10/10), c9559cf holds. Non-blocking leftovers: F4/F5/F6/F7 — see RESULTS/2026-08-29-dbcache-layer-gate.md |
| FE Scroll Long-Last-Message Gate (@ 7022be1, uncommitted) | 2026-09-22 | ✅ **PASS** (3 committer decision notes) | Suite 52/52 ×2 (36 windowing + 16 component; 2.95s/3.19s); vite build exit 0 (3.87s); tsc 0 new-debt (30 baseline pre-existing); empirical baseline stash-check: symptom-1 **4/4 catchers FAIL pre-fix**, symptom-2 initial-anchor + overflowAnchor **FAIL pre-fix** (static prediction of "masked" disproven — LESSONS/2026-09-22); build artifacts TRACKED (afe93ad convention) → commit needs 7 artifact changes or explicit revert; headless gap: late height materialization untestable → manual smoke list in RESULTS; report-only, tree left in expected state |
