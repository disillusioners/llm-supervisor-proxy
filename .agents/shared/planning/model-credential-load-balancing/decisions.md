...
> **Round 3d (2026-08-25) — Phase-1 implementation feedback:** validation does NOT reject non-internal models carrying non-empty `Credentials` — when the list is non-empty, the same ref-validation rules (existence / weight>0 / ≤16 / no duplicate IDs / provider-match against `Credentials[0]`'s provider) apply as for internal models; the `Credentials` list is simply not REQUIRED for non-internal models. The plan Task 6 reject rule ("creds only meaningful for internal") was the outlier — the contract (technical-analysis.md) is silent on it, and runtime evidence contradicts it: the D3 provider probe at `pkg/ultimatemodel/handler.go:625-635` reads `modelCfg.CredentialID`/`Credentials` on the ultimate-external path; the Round-3 backward-compat matrix's ultimate-external row; and the non-internal `ModelConfig` fixture seeded with `Credentials: models.TestRefs(openaiCredID)` at `test/e2e_minimax_reasoning/harness_test.go:280-286` all require non-internal models to carry credentials without rejection.
